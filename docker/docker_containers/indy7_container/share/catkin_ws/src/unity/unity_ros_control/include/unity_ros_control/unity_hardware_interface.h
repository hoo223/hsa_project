...
class UnityIndy7 : public hardware_interface::RobotHW
{
public:
  UnityIndy7();
...
private:
  ...
  ros::Subscriber unity_indy7_state_sub;
  std_msgs::Float64MultiArray command;
  std::string con_type;
};

UnityIndy7::UnityIndy7()
: joint_position_command_({ 0, 0, 0, 0, 0, 0 })
, joint_velocity_command_({ 0, 0, 0, 0, 0, 0 })
, joint_positions_{ { 0, 0, 0, 0, 0, 0 } } 
, joint_velocities_{ { 0, 0, 0, 0, 0, 0 } }
, joint_efforts_{ { 0, 0, 0, 0, 0, 0 } }
, joint_names_( {"joint0", "joint1", "joint2", "joint3", "joint4", "joint5"} ) 
...
{ 
  ...
  position_command_pub = nh.advertise<std_msgs::Float64MultiArray>("unity_indy7_position_command", 100);
  velocity_command_pub = nh.advertise<std_msgs::Float64MultiArray>("unity_indy7_velocity_command", 100);
  unity_indy7_state_sub = nh.subscribe<sensor_msgs::JointState>("unity_indy7_joint_states", 100, boost::bind(&UnityIndy7::stateCallback, this, _1));
}

void UnityIndy7::read(const ros::Time& time, const ros::Duration& period)
{
  joint_positions_ = unity_joint_positions_;
}

void UnityIndy7::write(const ros::Time& time, const ros::Duration& period)
{
...
void UnityIndy7::stateCallback(const sensor_msgs::JointStateConstPtr& msg)
{
...
void UnityIndy7::doSwitch(const std::list<hardware_interface::ControllerInfo>& start_list,
              const std::list<hardware_interface::ControllerInfo>& stop_list)
{
  ROS_INFO("switch");
...
bool UnityIndy7::checkControllerClaims(const std::set<std::string>& claimed_resources)
{
